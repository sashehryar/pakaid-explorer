/**
 * Core news-scraping logic — shared between:
 *   - app/api/scrape-news/route.ts  (cron + manual POST)
 *   - lib/scraper-handlers.ts       (admin "Trigger" button)
 *
 * No HTTP self-calls, no side-effects beyond DB.
 */
...
export function scoreRecency(pubDate: string | null): number {
...
function parseRSSXML(xml: string, feedUrl: string): FeedItem[] {
  ...
  for (const raw of rawItems.slice(0, 40)) {
    ...
    if (!link) continue
    const rawDesc = getTagText(raw, 'description') || getTagText(raw, 'summary') || ''
    items.push({
      title,
      link,
      description: stripHtml(rawDesc).slice(0, 800),
...
async function fetchFeed(feedUrl: string): Promise<FeedItem[]> {
  try {
    ...
    if (!resp.ok) return []
    return parseRSSXML(await resp.text(), feedUrl)
  } catch {
    return []
  }
}
...
async function fetchAllFeeds(
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      result.set(feed.feed_url, await fetchFeed(feed.feed_url))
    }
  }
...
// ── Main entry point ────────────────────────────────────────────────────────────
export async function runNewsScraperCore(): Promise<NewsScrapeResult | { error: string; detail: unknown }> {
  ...
  for (const feed of feeds) {
    const items = (allItems.get(feed.feed_url) ?? []).slice(0, PER_SOURCE_CAP)
    let feedInserted = 0

    if (items.length === 0) {
      errors.push(`${feed.feed_name}: 0 items fetched`)
      await supabase.from('news_feeds').update({
        last_fetched_at: new Date().toISOString(), last_item_count: 0,
      }).eq('id', feed.id)
      continue
    }
    ...
    try {
      for (const item of items) {
        ...
        const { error: upsertErr } = await supabase
          ...
          .upsert({
            ...
            relevance_country: isPakistan ? 'PK' : null,
            feed_category:     (feed as { category?: string }).category ?? null,
      ...
      await supabase.from('news_feeds').update({
        last_fetched_at: new Date().toISOString(), last_item_count: feedInserted,
      }).eq('id', feed.id)
  ...
  return { feeds_processed: feeds.length, feeds_total: allFeeds.length, feeds_with_data: feedsWithData, articles_inserted: totalInserted, articles_summarized: totalSummarized, errors }
}
